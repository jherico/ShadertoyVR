...
class MatrixStack : public std::stack<mat4> {
...
public:
  ...
  explicit MatrixStack(const MatrixStack & other) {
    *((std::stack<mat4>*)this) = *((std::stack<mat4>*)&other);
  }
  ...
  MatrixStack & push(const mat4 & mat) {
    std::stack<mat4>::push(mat);
    return *this;
  }

  MatrixStack & rotate(const glm::mat3 & rotation) {
    return postMultiply(mat4(rotation));
  }
  ...
  MatrixStack & rotate(float theta, const glm::vec3 & axis) {
    return postMultiply(glm::rotate(mat4(), theta, axis));
  }
  ...
  MatrixStack & translate(const glm::vec3 & translation) {
    return postMultiply(glm::translate(mat4(), translation));
  }

  MatrixStack & preTranslate(const glm::vec3 & translation) {
    return preMultiply(glm::translate(mat4(), translation));
  }
  ...
  MatrixStack & scale(const glm::vec3 & scale) {
    return postMultiply(glm::scale(mat4(), scale));
  }

  MatrixStack & transform(const mat4 & xfm) {
    return postMultiply(xfm);
  }

  MatrixStack & preMultiply(const mat4 & xfm) {
    top() = xfm * top();
    return *this;
  }

  MatrixStack & postMultiply(const mat4 & xfm) {
    top() *= xfm;
    return *this;
  }
